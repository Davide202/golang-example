...
const (
	queryInsertUser             = "INSERT INTO users(id,first_name, last_name, email, date_created, status, password) VALUES(?, ?, ?, ?, ?, ?, ?);"
...
func (user *User) Get() rest_errors.RestErr {
	...
	var id string = strconv.FormatInt(user.Id, 10)
	if getErr := result.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); getErr != nil {
		logger.Error("error when trying to get user by id "+id, getErr)
		return rest_errors.NewInternalServerError("error when tying to get user", errors.New("database error"))
	}
	return nil
}
...
func (user *User) Save() rest_errors.RestErr {
	...
	_, saveErr := stmt.Exec(user.Id, user.FirstName, user.LastName, user.Email, user.DateCreated, user.Status, user.Password)
	if saveErr != nil {
